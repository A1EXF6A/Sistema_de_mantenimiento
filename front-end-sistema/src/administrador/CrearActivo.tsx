...
import './CrearActivo.css'; // Importa el archivo CSS

const CrearActivo: React.FC = () => {
    const { tipo } = useParams<{ tipo: string }>(); // Obtenemos el parámetro "tipo" de la URL
    const [formData, setFormData] = useState({
        id_act: '2', // Valor predeterminado
        registrado_por: 1, // Valor predeterminado -> aqui va jw
        nombre: '',
        marca: '',
        modelo: '',
        serie: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('http://localhost:8000/api/registro-activos', {
                method: 'POST',
                headers: {
            ...
            alert('Ocurrió un error al crear el activo');
            console.error(error);
        }
    };
        ...
        <div className="crear-activo-container">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-field">
                    <Combobox
                        label="Ubicación"
                        endpoint="http://localhost:8000/api/ubicaciones"
                        valueField="id_ubi"
                ...
                <div className="form-field">
                    <Combobox
                        label="Responsable"
                        endpoint="http://localhost:8000/api/responsables"
                        valueField="id_res"
                ...
                <div className="form-field">
                    <Combobox
                        label="Proveedor"
                        endpoint="http://localhost:8000/api/proveedores"
                        valueField="id_pro"
